...
class LanguageLimitForm(ModelForm):
    ...
    def clean_time_limit(self):
        ...
        if (
            timelimit
            and timelimit > settings.CLAOJ_PROBLEM_TIMELIMIT_LIMIT
...
class ProblemEditForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            self.user
            and not self.user.has_perm("judge.change_public_visibility")
            and not org_pk
        ):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "description": MartorWidget(
                attrs={
                    "data-markdownfy-url": reverse_lazy("problem_preview"),
        ...
        help_texts = {
            ...
            "memory_limit": _(
                "The memory limit for this problem, in KB (e.g. 512 MB = 524288 KB)."
            ),
        }
        error_messages = {
            "code": {
...
class ProblemSubmitForm(ModelForm):
    source = CharField(
        max_length=65536,
        required=False,
        widget=AceWidget(theme="tomorrow", no_ace_media=True),
    )
