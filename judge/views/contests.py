...
class ContestDetail(ContestMixin, TitleMixin, CommentedDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        contest_problem_fields = (
            self.object.contest_problems.defer("problem__description")
            .order_by("order")
            .annotate(
                user_count=Count(
                    "submission__submission",
        ...
        for p, contest_p in zip(context["contest_problems"], contest_problem_fields):
            ...
            if contest_p["submission_count"]:
                p.ac_rate = (
                    contest_p["user_count"] / contest_p["submission_count"] * 100
                )
            else:
                p.ac_rate = 0
        ...
        context["attempted_problem_ids"] = (
            user_attempted_ids(self.request.profile) if authenticated else []
        )
...
def contest_ranking_list(contest, problems):
    return base_contest_ranking_list(
        contest, problems, base_contest_ranking_queryset(contest)
    )
...
class ContestRankingBase(ContestMixin, TitleMixin, DetailView):
    ...
    def get_rendered_ranking_table(self):
        ...
        return loader.render_to_string(
            self.ranking_table_template_name,
            context={
...
class ContestRanking(ContestRankingBase):
    ...
    def cache_key(self):
        return (
            f"contest_ranking_cache_{self.object.key}_{self.show_virtual}_"
    ...
    def bypass_cache_ranking(self):
        return (
            ...
            or self.can_edit
            or not self.object.can_see_full_scoreboard(self.request.user.profile)
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["display_first_solves"] = self.object.format.name not in [
            IOIContestFormat.name,
            LegacyIOIContestFormat.name,
        ]
        return context
