    ...
    def dispatch(self, request, *args, **kwargs):
        if self.has_permission():
            if (
                ...
                >= settings.CLAOJ_ORGANIZATION_ADMIN_LIMIT
...
class OrganizationHome(TitleMixin, CustomOrganizationMixin, PostListBase):
    ...
    def get_context_data(self, **kwargs):
        ...
        if (
            context["is_member"]
            or self.request.user.has_perm("judge.see_organization_problem")
        ...
        if (
            context["is_member"]
            or self.request.user.has_perm("judge.see_private_contest")
