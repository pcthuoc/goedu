...
def parse_tests(problem_meta, root, package):
    ...
    for i, test in enumerate(testset.find('tests').getchildren(), start=1):
        ...
        output = subprocess.check_output(["./sol"], input=package.read(input_path), text=False)  # Chạy lệnh và lấy output
        tests_zip.writestr(f'{i:02d}.out', output)  # Ghi output vào tệp zip
...
def extract_zip_file(zip_file):
    zip_file.extractall("/problems/up_de/giainen/")
    source_file = "/problems/up_de/giainen/solutions/sol.cpp"
    destination_file = "/problems/up_de/sol.cpp"

    shutil.copyfile(source_file, destination_file)
    print("Đã sao chép tệp soll.cpp thành công.")
    shutil.rmtree("/problems/up_de/giainen/")
    call(["g++", "-g", "-O2", "-std=c++17", "/problems/up_de/sol.cpp", "-o", "sol"])
